    ...
    public static class ConvertServiceConverter
    {
        public static void Convert(ConvertService.LogAction log, List<ModelFile> models, ConvertServiceParameters parameters)
        {
            ...
            foreach (var file in models)
            {
                ...
                string tempFilePath = Path.Combine(di.FullName, tmpfileName);
                string targetFilePath = Path.Combine(di.FullName, fileName);
                processedFiles.Add(targetFilePath);

                using (var stream = GetStream(tempFilePath, 0, log))
                {
                    using var f = new StreamWriter(stream, System.Text.Encoding.UTF8, 1024, false);
                ...
                if (!File.Exists(targetFilePath) || !File.ReadLines(targetFilePath).SequenceEqual(File.ReadLines(tempFilePath)))
                {
                    MoveFile(tempFilePath, targetFilePath);
                    log("Info: File saved successfully ({0})", targetFilePath);
                }
                else
                {
            ...
            var allFiles = Directory.GetFiles(parameters.ConvertDirectory, "*", SearchOption.AllDirectories).Select(f => Path.GetFullPath(f));
            var fileDeletedcount = 0;
            foreach (var file in allFiles.Where(f => !processedFiles.Contains(f)))
            {
                File.Delete(file);
                fileDeletedcount++;
            }

            log("Unchanged files: {0}", notChangedCount);
            log("Deleted files: {0}", fileDeletedcount);
        }

        private static FileStream GetStream(string saveDir, int iteration, ConvertService.LogAction log)
        {
            try
            {
            ...
            catch (UnauthorizedAccessException) when (iteration < 10)
            {
                ...
                return GetStream(saveDir, ++iteration, log);
            }
            ...
            {
                log("Open file stream failed, Path: {0}; Message: {1}", saveDir, ex.Message);
                throw;
            }
        }
