        ...
        public static void StartService(string[] args)
        {
            var convertService = new ConvertService((logString, logArgs) => Console.WriteLine(logString, logArgs),
                new ConvertServiceParameters()
                {
                    ...
                    IsDictAsPropertyBagOf = args.Length >= 8 ? bool.Parse(args[7]) : false,
                    PathOfPropertyBagOf = args.Length >= 9 ? args[8] : "",
                    Implements = args.Length >= 10? CreateDictionary(args[9]) : new Dictionary<string, (string type, string from)>(),
                    Extends = args.Length >= 11 ? CreateDictionary(args[10]) : new Dictionary<string, (string type, string from)>(),
                });
